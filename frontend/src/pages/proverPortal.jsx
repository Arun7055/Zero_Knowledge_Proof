...
import { Scanner } from "@yudiel/react-qr-scanner"; // <-- Added Scanner Import
...
export default function ProverPortal() {
  ...
  const [showScanner, setShowScanner] = useState(false); // <-- Added Scanner State
      ...
      {activeTab === "wallet" && (
        <>
          <div className="mb-8 p-6 bg-slate-50 border border-slate-200 rounded-lg">
            ...
            <p className="text-sm text-slate-600 mb-4">Paste the Request ID or scan the QR code provided by your employer or insurer.</p>
            
            {/* Camera Scanner UI */}
            {showScanner && (
              <div className="mb-4 p-4 bg-slate-900 rounded-lg relative overflow-hidden flex flex-col items-center">
                <button 
                  type="button"
            ...
            <form onSubmit={handleLoadRequest} className="flex gap-2 mt-4">
              <input required value={reqIdInput} onChange={e => setReqIdInput(e.target.value)} className="border p-2 rounded flex-1 focus:outline-none focus:border-medical-blue" placeholder="e.g., req_17000000..." />
